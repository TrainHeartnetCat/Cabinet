...
with open(r"D:\Works\backup\Project01_ErrorEstimate_DRM_on_Ni111\1_ouput_data\Raw_output\AE_error_with_order_923_1023.json", "r") as f:
    AE_error_data = json.load(f)

with open(r"D:\Works\backup\Project01_ErrorEstimate_DRM_on_Ni111\1_ouput_data\Raw_output\RE_error_with_order_923_1023.json", "r") as f:
...
for key in data:
    ...
    order_to_log_rate, \
    rmse, \
    order_to_error_of_AE, \
    order_to_error_of_RE = data[key]


# TODO 加上order2actRMSE，ord2reaRMSE
    ...
    for i in range(2000):
        try:
            ...
            target_AE_error = order_to_error_of_AE[i]
            target_RE_error = order_to_error_of_RE[i]
        ...
        # TODO error_mv_to_RMSE_of_activation
...
def mv_to_accuracy_plot(mv_to_label_dict,  # mean和var作为key，预测结果作为value
    ...
    for key in mv_to_label_dict:
        ...
        pos_data[x, y].extend(mv_to_label_dict[key])
    ...
    for xy in pos_data:
        ...
        for h in hashs:
            errors_.append(h)
        # pos_accuracy[xy] = np.mean(errors_)
        # pos_accuracy[xy] = np.var(errors_)
    ...
    if plot_accuracy_or_RMSE_distrib:
        ...
        plt.imshow(pos_accuracy, cmap="GnBu")
 ...
 17.09090909, 17.27272727, 17.45454545, 17.63636364, 17.81818182, 18.        , 18.181819, 18.363637]), lw=LW_, c="black",alpha=line_alpha)
        ...
        # plt.xlim(-0.5,resolution+0.8)
        plt.xlim(-0.5,resolution+1)
        plt.ylim(-1.2,19.2)


        new_ticksy = np.linspace(-0.2,18.2, 5)
        plt.yticks(new_ticksy)
        # new_ticksx = np.linspace(-0.5,resolution+1, 5)
        # plt.xticks(new_ticksx)
...
if __name__ == '__main__':
    ...
    DATA_SETTINGS = {
        "AE2d_RMSE": dict(data_source=error_mv_to_ae_RMSE,
                     correct_label_source={923:0, 973:0, 1023:0},
                     title="AE_RMSE", colorbar_range=(0,0.6)),
        "RE2d_RMSE": dict(data_source=error_mv_to_re_RMSE,
                     correct_label_source={923:0, 973:0, 1023:0},
                     title="RE_RMSE", colorbar_range=(0,0.6)),
    }
    ...
    #     for keys in DATA_SOURCE[key]:

    #         print(len(DATA_SOURCE[key][keys]))
...
# TODO 做一维AE，RE随RMSE变化
# color bar 的精度可能需要调整，RE和AE已经体现不出差别了，几乎完全一样的图




    # 把处理过的AE和RE的RMSE打包出来，方便其他代码修改和查错
    # output_path = "D:/Works/backup/ErrorEstimate/Running_analysis/data_collect"# change
    # Key_mean_var_RMSE_AE_pkl = output_path + '/' + 'Key_mean_var_RMSE_AE.pkl'
    # # Key_mean_var_RMSE_AE_json = output_path + '/' + 'Key_mean_var_RMSE_AE.json'
    # Key_mean_var_RMSE_AE_txt = output_path + '/' + 'Key_mean_var_RMSE_AE.txt'

    # with open(Key_mean_var_RMSE_AE_pkl, "wb") as f:
    #     pickle.dump(DATA_SOURCE, f)
    # # with open(Key_mean_var_RMSE_AE_json, "w") as f:
    # #     json.dump(DATA_SOURCE, f)
    # with open(Key_mean_var_RMSE_AE_txt, "w") as f:
